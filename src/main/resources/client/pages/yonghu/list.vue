...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	...
	<view :style='{"width":"80%","padding":"120rpx 20rpx","background":"#d1f1ed"}' class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
		<view :style='{"width":"100%","padding":"20rpx 20rpx 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
			<view :style='{"width":"18%","padding":"0 0 0 20rpx","fontSize":"24rpx"}'>身份证</view>
			<input :style='{"width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","color":"#666666","background":"#ffffff","height":"60rpx"}' placeholder="请输入身份证" v-model="searchForm.shenfenzheng">
		</view>
		<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
			<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>是否户主</view>
			<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
				<view @click="shifouhuzhuChange(item)" v-for="(item,index) in shifouhuzhuOptions" :key="index" class="screenTab" :class="searchForm.shifouhuzhu==item?'screenTabActive':''">{{item}}</view>
			</view>
		</view>
		<view :style='{"width":"100%","padding":"20rpx 20rpx 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
			<view :style='{"width":"18%","padding":"0 0 0 20rpx","fontSize":"24rpx"}'>常驻人口</view>
			<input :style='{"width":"80%","padding":"0 20rpx 0 20rpx","borderRadius":"40rpx","color":"#666666","background":"#ffffff","height":"60rpx"}' placeholder="请输入常驻人口" v-model="searchForm.changzhurenkou">
		</view>
		<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
			<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>特殊人群</view>
			<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
				<view @click="teshurenqunChange(item)" v-for="(item,index) in teshurenqunOptions" :key="index" class="screenTab" :class="searchForm.teshurenqun==item?'screenTabActive':''">{{item}}</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			shifouhuzhuChange(e){
				this.searchForm.shifouhuzhu = e
				this.$forceUpdate()
			},
			teshurenqunChange(e){
				this.searchForm.teshurenqun = e
				this.$forceUpdate()
			},
			screenReset(){
				this.searchForm = {}
			...
			queryChange(e) {
				...
				this.searchForm.shifouhuzhu="";
				this.searchForm.changzhurenkou="";
				this.searchForm.teshurenqun="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.shifouhuzhu){
					params['shifouhuzhu'] = '%' + this.searchForm.shifouhuzhu + '%'
				}
				if(this.searchForm.changzhurenkou){
					params['changzhurenkou'] = '%' + this.searchForm.changzhurenkou + '%'
				}
				if(this.searchForm.teshurenqun){
					params['teshurenqun'] = '%' + this.searchForm.teshurenqun + '%'
				}
			...
			async search(){
				...
				if(this.searchForm.shifouhuzhu){
					searchForm['shifouhuzhu'] = this.searchForm.shifouhuzhu
				}
				if(this.searchForm.changzhurenkou){
					searchForm['changzhurenkou'] = '%' + this.searchForm.changzhurenkou + '%'
				}
				if(this.searchForm.teshurenqun){
					searchForm['teshurenqun'] = this.searchForm.teshurenqun
				}
                let res = {};
                if(this.userid) {
