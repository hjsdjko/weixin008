	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussfuwuzhan`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussfuwuzhan`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussfuwuzhan`, this.ruleForm);
						}else{
							await this.$api.add(`discussfuwuzhan`, this.ruleForm);
						}
						uni.setStorageSync('discussfuwuzhanCleanType',true);
