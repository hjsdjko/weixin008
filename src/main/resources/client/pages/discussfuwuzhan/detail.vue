...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfuwuzhanCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfuwuzhanCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussfuwuzhan', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussfuwuzhan');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussfuwuzhan');
			...
			async init(){
				...
				let res = await this.$api.info('discussfuwuzhan', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussfuwuzhan', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussfuwuzhan/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
